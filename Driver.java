...
public class Driver extends NavigationMethod
{
	...
	private static int runtime = 3000;

	private static final int Right_channel = 4; 
	private static final int Left_channel = 5;

	private static int right_forward_speed = -800; // Speed of port motor.
	private static int left_forward_speed = -200; // Speed of starboard of motor.
...

	{
		...
		boolean Ping_Test = FALSE; 
		boolean Servo_Motor_Test = FALSE; 
		boolean Temperature_Test = TRUE; 

		
//		r.runTwoPCAMotor(Right_channel, right_forward_speed, Left_channel, left_forward_speed, runtime);

//		r.runPCAMotor(Right_channel, right_forward_speed, runtime);

//		r.runPCAMotor(Left_channel, left_forward_speed, runtime);
...
//		RunOneMotor.runForward(Right_channel, right_forward_speed, Left_channel, left_forward_speed, runtime);
		...
		if (Servo_Motor_Test == TRUE) {
			driver4(); /////////////////////////// FIXME!!! Currently set to "Actuator"
		}
	...
	public static void driver1() {
		...
		if (relative_orientation == 90) {
			runtime = forward_runtime;
			Driver.runForward(Right_channel, right_forward_speed, Left_channel, left_forward_speed, runtime);
		}
